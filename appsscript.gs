...
// ======== PDF RETURN (data URI download page) ========
function returnPdf(html, filename) {
  try {
    var blob = HtmlService.createHtmlOutput(html)
      .setTitle(filename).getBlob()
      .setName(filename).setContentType("application/pdf");
    var b64 = Utilities.base64Encode(blob.getBytes());

    return HtmlService.createHtmlOutput(
      '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Talent Nexus</title>' +
      '<style>body{font-family:"Segoe UI",Arial,sans-serif;text-align:center;padding:40px;color:#1a1a2e;margin:0}' +
      'h2{font-size:18px;color:#1a1a2e;margin:0 0 4px}hr{width:48px;height:2px;background:#c9a84c;border:0;margin:8px auto 16px}' +
      '.card{display:inline-block;border:1px solid #e0e0e0;border-radius:10px;padding:24px 32px;box-shadow:0 2px 12px rgba(0,0,0,0.06)}' +
      '.btn{display:inline-block;padding:12px 32px;background:#1a1a2e;color:#fff;border-radius:6px;text-decoration:none;font-weight:600;font-size:14px;margin:12px 0}' +
      '.btn:hover{background:#2d2d4a}p.note{font-size:11px;color:#999;margin-top:14px}</style></head><body>' +
      '<div class="card"><h2>TALENT NEXUS</h2><hr><p style="font-size:14px;color:#666">Your document is ready.</p>' +
      '<p><a class="btn" href="data:application/pdf;base64,' + b64 + '" download="' + filename + '">Download PDF</a></p>' +
      '<p class="note">If download does not start, click the button above.<br>Generated by Talent Nexus HR &bull; ' + HR_NAME + '</p></div>' +
      '<script>setTimeout(function(){var a=document.querySelector(".btn");if(a)a.click()},600);</script>' +
      '</body></html>'
    );
  } catch (e) {
    return HtmlService.createHtmlOutput(
